...
const Prototypes = () => {
    ...
    Object.defineProperty(Array.prototype, 'removeLeavingGroupX', {
        value: function(logger) {
            ...
            try {
                ...
                logger.log('info','Prototype.removeLeavingGroup() ' + e)
                return false
            }
            ...
            try {
                if (leaving_group_atom.carbonBonds(atoms).length === 0) {
            ...
            leaving_group_atom.carbonBonds(atoms).length.should.be.greaterThan(0)
            const parent_carbon = leaving_group_atom.carbonBonds(atoms)[0].atom
           ...
           // leaving_group_atom.breakBond(parent_carbon, molecule, logger)
            ...
            try {
                if(leaving_group_atom.charge(leaving_groups[0],logger) !== 0){
                    throw new Error("Removal of leaving group should have resulted in a positive charge on the leaving group atom")
                }
            } catch(e) {
                console.log('Leaving group atom')
                console.log(leaving_group_atom)
                console.log('Charge')
                leaving_group_atom.charge(leaving_groups[0], logger)
                console.log(e)
                process.exit()
            }
    ...
    // Remove
    Object.defineProperty(Array.prototype, 'checkBonds', {
               ...
               // Break bond by "collapsing" electron pair onto "atom"
                ...
                try {
                    if (false ===atom.isCarbocation(molecule.atoms, logger)) {
                        throw new Error('Prototypes.removeBondFromCarbon() Failed to create a carbocation')
                    }
                } catch(e) {
    ...
    Object.defineProperty(Array.prototype, 'makeDativeBond', {
            ...
            try {
                ...
                Typecheck(
                    ...
                    */
                    ...
                    // Base atom atoms should have at least 2 free electrons as otherwise we cannot push
                    // and electron pair from the base atom to the target atom.
