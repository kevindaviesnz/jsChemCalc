...
const GetMechanismThatCouldHaveBeenUsedToGetCurrentSubstrateAndAddToReactionSteps = (
    ...
    try {
        ...
        _.cloneDeep(mechanisms).map((mechanism, i) => {
            ...
            // let last_mechanism = undefined === mechanisms[mechanisms.length-1]?
             const matching_reaction_step_found = (reaction_steps, container) =>{
                 const matching_reaction_step = _.find(reaction_steps, (r)=>{
                     ...
                     return matching_reverse_mechanism === mechanism_names[i] && r.precursor_substrate_SMILES === container.getSubstrate()[0].smiles_string
                })
            ...
            // logger.log('debug', '[Synthesise] pathway number ' + pathway_number)
             const pathway_numbers = pathway_number.split('.')
             pathway_numbers[pathway_numbers.length-1] = i
             pathway_number = pathway_numbers.join('.')

            // logger.log('debug', '[Synthesise] pathway number (after incrementing) ' + pathway_number)
             ...
             return mechanism(
                 ...
                 (container, mechanism_name, computed_previous_container_state, synth, mechanism, max_steps_copy, pathway_id, logger) => {
                     ...
                     if (false) {
                         ...
                         LookupMolecule(db, computed_previous_container_state.substrate.smiles_string, "SMILES", logger, PubChemLookup, [pathway_id]).then(
                             // "resolves" callback
                             (parent_chemical) => {
                                 let last_reaction_step = null
                                 LookupMolecule(db, container.getSubstrate()[0].smiles_string, "SMILES", logger, PubChemLookup).then(
                                     (child_chemical) => {
                                        /*
                                        ...
                                        */
                                        let product_canonicalSMILES = container.getSubstrate()[0].smiles_string
                                        ...
                                        if ('dehydration' === reaction_step.mechanism && 'dehydration' === reaction_steps[reaction_steps.length-1].mechansism) {
                                            logger.log('info', ('[Synthesise] IGNORE 998 Not adding reaction step as it is dehydration and last reaction step was dehydration').bgRed)
                                        } else {
