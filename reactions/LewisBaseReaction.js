...
/**
 * Perform Lewis Acid Base reaction using substrate as the base.
 * 
 * @param container 
 * @param logger 
 * 
 * @return {null} 
 */
const LewisBaseReaction = (container, logger) => {
  try {
    Typecheck(
    ...
    const reagent = FindStrongestLewisAcidMolecule(nonGenericReagents, logger);
    if (undefined === reagent) {
      // Can't use reagent as a Lewis Acid
      return false;
    }

    const lewisBaseAtom = LewisBaseAtom(container.getSubstrate()[0]);
    const lewisAcidAtom = LewisAcidAtom(reagent);
...
module.exports = LewisBaseReaction;
